...
public class Code02_ExpressionNumber {
	...
	public static int f(char[] str, boolean desired, int L, int R) {
		if (L == R) { // base case 1
			if (str[L] == '1') {
				//如果期待为true的话 就有一种方法 如果期待为false那么方法数就是0
				return desired ? 1 : 0;
			} else { // '0'
				return desired ? 0 : 1;
			}
	...
	public static int dpLive(String express, boolean desired) {
		...
		//根据basecase
		//先填对角线位置 每次跳两个是因为奇数位必是运算符号不能填1或0
		for (int i = 0; i < N; i += 2) {
			tMap[i][i] = str[i] == '1' ? 1 : 0;
			fMap[i][i] = str[i] == '0' ? 1 : 0;
		}
		//因为二维表最后一行 N-1行 除了右下角的位置其他位置都是弃用的位置 而对角线位置已经填过了
		//又因为N-2行是偶数行 弃用 所以从N-3行开始填  row -= 2 跳过偶数行
		//而对角线上已经填过了 往右一个位置是偶数列 所以对角线从往右的第2个位置开始往右填
