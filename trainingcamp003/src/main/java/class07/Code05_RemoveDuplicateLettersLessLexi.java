...
public class Code05_RemoveDuplicateLettersLessLexi {
	...
	public static String removeDuplicateLetters1(String str) {
		...
		int minACSIndex = 0;
		for (int i = 0; i < str.length(); i++) {
			//在从左往右遍历删除的过程中收集最小的ACS字符所在的位置
			//如果当前位置的ACS码小于之前收集的位置上的字符的acs码
			//那么minACSIndex 就更新到当前位置
			//只有当前字符更小时才更新 所以如果有多个最小字符记录的一定是最左的位置
			minACSIndex = str.charAt(minACSIndex) > str.charAt(i) ? i : minACSIndex;
			//当词频表中某个字符的词频被减到了0 停止
		...
		//最终返回结果字符串
		return  //选则保留的字符
				String.valueOf(str.charAt(minACSIndex)) +
				//将选则保留的字符的右侧字符串 进入递归 继续选择下一个需要保留的字符
				removeDuplicateLetters1(
				//并将右侧所有和以选择字符相同的字符都替换成"" 相当于删除
