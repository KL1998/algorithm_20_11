...
public class Problem_0309_BestTimeToBuyAndSellStockWithCooldown {
	...
	public static int maxProfit2(int[] prices) {
		...
		for (int i = 2; i < N; i++) {
			//可能性1) i位置不是最后一次的买入时机 那么i-1 怎么买 i就怎么买  buy[i - 1]
			//可能性2) i位置是最后一次的买入时机  因为有冻结期所以上一次交易一定发生在 0 ~ i-2的范围上
			buy[i] = Math.max(buy[i - 1], sell[i - 2] - prices[i]);
			//可能性1) i位置不是最后一次的卖出时机 那么i - 1 怎么卖 i就怎么卖 sell[i - 1]
			//可能性2) i位置是最后一次的卖出时机 最后一次的买入动作的最好情况 再加上当前i的价格
			sell[i] = Math.max(sell[i - 1], buy[i - 1] + prices[i]);
		}
		//最终返回 再在0...N - 1范围上，最后一次动作是sell的情况下，最好的收益
		return sell[N - 1];
	}
