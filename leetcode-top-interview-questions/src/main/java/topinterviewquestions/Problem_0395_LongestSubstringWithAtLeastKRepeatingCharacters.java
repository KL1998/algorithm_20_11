...
public class Problem_0395_LongestSubstringWithAtLeastKRepeatingCharacters {
	...
	public static int longestSubstring2(String s, int k) {
		...
		for (int require = 1; require <= 26; require++) {
			...
			// L要尝试每一个窗口的最左位置
			for (int L = 0; L < N; L++) {
				//R + 1 < N R + 1位置不越界  表示后面还有字符 R有必要向右扩
				//collect == require 种数收集够了
				//count[str[R + 1] - 'a'] == 0 R+1位置上的字符 在窗口中没有出现过 R如果再向右扩 种数就会超了
				//不越界 下一个位置的字符进来 种数不超 R就往右扩
				...
				// L++ 如果当L往右扩 此时 即将要出窗口的字符只收集到1次
				//窗口内收集到的字符种数--
				if (count[str[L] - 'a'] == 1) {
					collect--;
				}
				//如果即将出去的字符在此时的窗口中 达到了k次 出去后就不达标了
				//目前窗口内出现次数>=k次的字符，满足了几种 --
				if (count[str[L] - 'a'] == k) {
					satisfy--;
				}
