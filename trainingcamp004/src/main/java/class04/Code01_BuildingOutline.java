...
public class Code01_BuildingOutline {
	...
	public static class Op {
		public int x; // x轴上的值
		public boolean isAdd;// true在x点增加了一个高度，false为减去了一个高度
		public int h; // 高度
	...
	public static class OpComparator implements Comparator<Op> {
		...
		public int compare(Op o1, Op o2) {
			...
			//如果一维数据一样 且操作符不一样 +号排在前 -号排在后
			if (o1.isAdd != o2.isAdd) {
				return o1.isAdd ? -1 : 1;
			}
			//如果一维数据一样 且操作符一样 谁在前谁在后都无所谓
			return 0;
		}
	}
	...
	// [s,e,h]
	// [s,e,h]
	//返回所有的轮廓线 ist<List<Integer>> ： { {1,5,3} , {6,8,4}  .. ...  ...    }
	public static List<List<Integer>> buildingOutline(int[][] matrix) {
		//一共多少大楼
		int N = matrix.length;
		//一共会有N*2个对象 每座大楼哪个位置加了一个高度 哪个位置减去了一个高度
		Op[] ops = new Op[N << 1];
		//生成所有大楼的对象
		...
		//生成轮廓线
		//最大高度变化的时候前一个轮廓线的结尾位置结算，新的轮廓线开始生成周而复始
